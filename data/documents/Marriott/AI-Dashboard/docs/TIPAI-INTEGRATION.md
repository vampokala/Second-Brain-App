...
%%{init: {'theme': 'dark', 'themeVariables': {'fontSize': '20px', 'primaryTextColor': '#f8fafc', 'lineColor': '#94a3b8', 'clusterBkg': '#111827', 'titleColor': '#f8fafc'}, 'flowchart': {'htmlLabels': true, 'useMaxWidth': true, 'padding': 22, 'nodeSpacing': 58, 'rankSpacing': 72}}}%%

flowchart LR

subgraph Guest["<b>Guest</b>"]
...
subgraph ECMP["<b>Chat/ECMP (Us)</b>"]

WH["<b>Webhook Receiver</b>"]

PII["<b>CC Redaction</b>"]
...
subgraph TIP["<b>TIP.AI (Them)</b>"]

API["<b>POST /v1/sessions/:id/messages</b>"]

CL["<b>Classification</b>"]

RAG["<b>RAG Assessment</b>"]

GEN["<b>Response Generation</b>"]
...
BOT["<b>Bot Auto-Response</b>"]

HUM["<b>Human Associate</b>"]
...
GEN -->|"<b>classification + assessment + draft</b>"| RT

RT -->|"<b>RAG-eligible + FULL</b>"| BOT

RT -->|"<b>Human route or NONE</b>"| HUM

RT --> DB

  

style G fill:#374151,stroke:#9ca3af,color:#f9fafb

style WH fill:#134e4a,stroke:#2dd4bf,color:#ecfdf5

style PII fill:#134e4a,stroke:#2dd4bf,color:#ecfdf5

style RT fill:#134e4a,stroke:#2dd4bf,color:#ecfdf5

style DB fill:#134e4a,stroke:#2dd4bf,color:#ecfdf5

style API fill:#14532d,stroke:#4ade80,color:#f0fdf4

style CL fill:#14532d,stroke:#4ade80,color:#f0fdf4

style RAG fill:#14532d,stroke:#4ade80,color:#f0fdf4

style GEN fill:#14532d,stroke:#4ade80,color:#f0fdf4

style BOT fill:#713f12,stroke:#facc15,color:#fefce8

style HUM fill:#1e3a8a,stroke:#60a5fa,color:#eff6ff
...
```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'fontSize': '15px', 'primaryTextColor': '#f8fafc', 'lineColor': '#94a3b8', 'clusterBkg': '#111827', 'titleColor': '#f8fafc'}, 'flowchart': {'htmlLabels': true}}}%%
flowchart TD
    MSG["<b>Guest Message (PII-redacted)</b>"] --> P1

    P1["<b>Phase 1: Pre-Scan</b><br/>(flags only, does not route)<br/>is_safety_urgent | is_complaint | is_pure_ack"]
    P1 --> P2

    P2{"<b>Phase 2: Guards</b><br/>(blocking)"}
    P2 -->|"<b>Payment PII</b>"| PAY["<b>BILL_PROB_DISPUTE</b><br/>method=PAYMENT_GUARD"]
    P2 -->|"<b>Non-Latin &gt;20%</b>"| LANG["<b>GENERAL_UNCLEAR</b><br/>method=LANG_GUARD_NONLATIN"]
    P2 -->|"<b>Pass</b>"| P3

    P3{"<b>Phase 3: Deterministic</b><br/>(first match wins)"}
    P3 -->|"<b>Safety / Button / Exact / Regex</b>"| DET["<b>Bucket locked</b><br/>method=DETERMINISTIC"]
    P3 -->|"<b>No match</b>"| P4

    P4{"<b>Phase 4: Pure Ack</b><br/>Detection"}
    P4 -->|"<b>Pure ack</b>"| ACK["<b>GENERAL_ACK</b><br/>method=PURE_ACK"]
    P4 -->|"<b>Has request signals</b>"| P5

    P5{"<b>Phase 5: Followup</b><br/>Inference"}
    P5 -->|"<b>Numeric qty/time</b>"| FOL["<b>Inherit prev bucket</b><br/>method=DETERMINISTIC_QTY/TIME"]
    P5 -->|"<b>Same/ditto</b>"| SAME["<b>Inherit prev bucket</b><br/>method=DETERMINISTIC_SAME"]
    P5 -->|"<b>No match</b>"| P6

    P6["<b>Phase 6: LLM Classification</b><br/>(fallback)<br/>Claude Sonnet via LiteLLM"]
    P6 --> P7

    P7{"<b>Phase 7: Post-Guards</b><br/>(sanity checks)"}
    P7 -->|"<b>Pass</b>"| DONE["<b>Final bucket_key</b>"]
    P7 -->|"<b>Fail</b>"| DOWN["<b>GENERAL_UNCLEAR</b><br/>method=*_SANITY_DOWNGRADE"]

    style MSG fill:#374151,stroke:#9ca3af,color:#f9fafb
    style P1 fill:#134e4a,stroke:#2dd4bf,color:#ecfdf5
    style P2 fill:#713f12,stroke:#facc15,color:#fefce8
    style P3 fill:#14532d,stroke:#4ade80,color:#f0fdf4
    style P4 fill:#14532d,stroke:#4ade80,color:#f0fdf4
    style P5 fill:#14532d,stroke:#4ade80,color:#f0fdf4
    style P6 fill:#1e3a8a,stroke:#60a5fa,color:#eff6ff
    style P7 fill:#713f12,stroke:#facc15,color:#fefce8
    style PAY fill:#7f1d1d,stroke:#f87171,color:#fef2f2
    style LANG fill:#7f1d1d,stroke:#f87171,color:#fef2f2
    style DET fill:#14532d,stroke:#4ade80,color:#f0fdf4
    style ACK fill:#14532d,stroke:#4ade80,color:#f0fdf4
    style FOL fill:#14532d,stroke:#4ade80,color:#f0fdf4
    style SAME fill:#14532d,stroke:#4ade80,color:#f0fdf4
    style DONE fill:#14532d,stroke:#4ade80,color:#f0fdf4
    style DOWN fill:#7f1d1d,stroke:#f87171,color:#fef2f2
```

### Future: Single TIP.AI Call

```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'fontSize': '16px', 'primaryTextColor': '#f8fafc', 'lineColor': '#94a3b8', 'clusterBkg': '#111827', 'titleColor': '#f8fafc'}, 'flowchart': {'htmlLabels': true}}}%%
flowchart LR
    MSG["<b>Guest Message</b>"] --> PII["<b>ECMP: CC Redaction</b>"]
    PII --> TIPAI["<b>POST /v1/sessions/:id/messages</b><br/>(TIP.AI applies PII redaction)"]
    TIPAI --> RESP["<b>TIP.AI Response:</b><br/>bucket_key + confidence +<br/>answerability + draft_response"]
    RESP --> RT["<b>ECMP: Route Decision</b>"]

    style MSG fill:#374151,stroke:#9ca3af,color:#f9fafb
    style PII fill:#134e4a,stroke:#2dd4bf,color:#ecfdf5
    style TIPAI fill:#14532d,stroke:#4ade80,color:#f0fdf4
    style RESP fill:#14532d,stroke:#4ade80,color:#f0fdf4
    style RT fill:#134e4a,stroke:#2dd4bf,color:#ecfdf5
```
...
%%{init: {'theme': 'dark', 'themeVariables': {'fontSize': '15px', 'primaryTextColor': '#f8fafc', 'lineColor': '#94a3b8', 'clusterBkg': '#111827', 'titleColor': '#f8fafc'}, 'flowchart': {'htmlLabels': true}}}%%
flowchart TD
    M1["<b>MSG 1: 'Pool hours?'</b><br/>bucket: AMEN_INFO_POOL<br/>method: DET_REGEX"] --> S1["<b>State:</b> prev=AMEN_INFO_POOL, turn=1"]
    S1 --> M2["<b>MSG 2: 'Thanks'</b><br/>bucket: GENERAL_ACK<br/>method: PURE_ACK"] --> S2["<b>State:</b> prev=AMEN_INFO_POOL (unchanged), turn=2"]
    S2 --> M3["<b>MSG 3: 'And breakfast?'</b><br/>LLM hint: prev was AMEN_INFO_POOL<br/>bucket: FNB_INFO_BREAKFAST"] --> S3["<b>State:</b> prev=FNB_INFO_BREAKFAST, turn=3"]
    S3 --> M4["<b>MSG 4: 'Can I get towels?'</b><br/>bucket: HK_REQ_SUPPLIES<br/>method: DET_REGEX"] --> S4["<b>State:</b> prev=HK_REQ_SUPPLIES, turn=4"]
    S4 --> M5["<b>MSG 5: '3'</b><br/>QTY followup from prev<br/>bucket: HK_REQ_SUPPLIES (Qty: 3)<br/>method: DETERMINISTIC_QTY"] --> S5["<b>State:</b> prev=HK_REQ_SUPPLIES, turn=5"]

    style M1 fill:#14532d,stroke:#4ade80,color:#f0fdf4
    style M2 fill:#374151,stroke:#9ca3af,color:#f9fafb
    style M3 fill:#1e3a8a,stroke:#60a5fa,color:#eff6ff
    style M4 fill:#14532d,stroke:#4ade80,color:#f0fdf4
    style M5 fill:#14532d,stroke:#4ade80,color:#f0fdf4
    ...
    style S5 fill:#1e293b,stroke:#64748b,color:#f1f5f9
```
...
%%{init: {'theme': 'dark', 'themeVariables': {'fontSize': '16px', 'primaryTextColor': '#f8fafc', 'lineColor': '#94a3b8', 'clusterBkg': '#111827', 'titleColor': '#f8fafc'}, 'flowchart': {'htmlLabels': true}}}%%
flowchart TD
    A["<b>Classified Message</b><br/>+ bucket_key"] --> B{"<b>1. Manual Override?</b>"}
    B -->|"<b>action = bot</b>"| BOT["<b>BOT MODE</b><br/>Generate draft"]
    B -->|"<b>action = handoff</b>"| HAND["<b>HANDOFF MODE</b><br/>Eval only"]
    B -->|"<b>auto / no override</b>"| C{"<b>2. Taxonomy Lookup</b>"}
    C -->|"<b>route = Human-Full</b><br/>OR rag_mode = No-RAG"| HAND2["<b>HANDOFF MODE</b><br/>Eval only"]
    C -->|"<b>RAG-eligible</b>"| BOT2["<b>BOT MODE</b><br/>Generate draft"]
    C -->|"<b>No taxonomy match</b>"| BOT3["<b>BOT MODE</b><br/>Generate draft (fallback)"]

    style A fill:#374151,stroke:#9ca3af,color:#f9fafb
    style BOT fill:#14532d,stroke:#4ade80,color:#f0fdf4
    style BOT2 fill:#14532d,stroke:#4ade80,color:#f0fdf4
    style BOT3 fill:#14532d,stroke:#4ade80,color:#f0fdf4
    style HAND fill:#1e3a8a,stroke:#60a5fa,color:#eff6ff
    style HAND2 fill:#1e3a8a,stroke:#60a5fa,color:#eff6ff
    style B fill:#713f12,stroke:#facc15,color:#fefce8
    style C fill:#134e4a,stroke:#2dd4bf,color:#ecfdf5
```
